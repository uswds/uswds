...
* Uses a **[modified BEM](https://frontend.18f.gov/css/naming/)** approach for naming CSS selectors. Objects in CSS are separated by single dashes. Modifier classes are additive — proper markup requires the base class _and_ the modifier class or classes. Modifier classes consist of the base class plus a modifier suffix, separated by two hyphens (`--`) as in `.usa-button.usa-button--secondary` or `usa-accordion.usa-accordion--borderless`.
