          ...
          describe(`on ${device.description}`, () => {
            ...
            if (process.env.ENABLE_SCREENSHOTS) {
              ...
              if (vrt.doesGoldenFileExist()) {
                it('matches golden screenshot', () => vrt.screenshot(cdp).then(vrt.ensureMatchesGoldenFile));
              } else {
                it('is the new golden screenshot', () => vrt.screenshot(cdp).then(vrt.saveToGoldenFile));
              }
            }
          });
        });
